...
class TestZip(BaseArchiveTestCase):
    ...
    expected_deeply_nested_relative_path_alternative = [
        ...
        "/sub/sub/sub/sub/sub/sub/a_parent_folder_in_sub_1.txt",
        ...
        "/sub/sub/sub/sub/sub/sub/sub/sub/sub/sub/sub/sub/a_parent_folder_in_sub_2.txt",
        "/sub/sub/sub/sub/sub/sub/sub/sub/sub/sub/sub/sub/a_parent_folder_in_sub_3.txt",
    ]
    ...
    def test_list_zip_with_relative_path_deeply_nested_with_7zip(self):
        ...
        expected = [
            ...
            },
            {
                ...
                "is_symlink": False,
                "link_target": None,
                "path": "../../../../../../../../../../../../sub/sub/sub/sub/sub/sub/a_parent_folder_in_sub_1.txt",
                "size": "9",
            },
            {
                "errors": [],
                ...
                "is_symlink": False,
                "link_target": None,
                "path": "../../../../../../sub/sub/sub/sub/sub/sub/sub/sub/sub/sub/sub/sub/a_parent_folder_in_sub_2.txt",
                "size": "9",
            },
            {
                "errors": [],
                ...
                "is_symlink": False,
                "link_target": None,
                "path": "../../../../../../../../../../../../sub/sub/sub/sub/sub/sub/sub/sub/sub/sub/sub/sub/a_parent_folder_in_sub_3.txt",
                "size": "9",
            },
        ]
        assert expected == result
    ...
    def test_extract_zip_with_duplicated_lowercase_paths(self):
        ...
        expected = {
            "META-INF/license/": None,  # a directory
    ...
    def test_extract_zip_with_timezone_2(self):
        ...
        expected = [
            (
                os.path.join(test_dir, "primes.txt"),
                (
                    "2009-12-05",
                    "2009-12-06",
                ),
            ...
            (
                os.path.join(test_dir, "primes2.txt"),
                (
                    "2009-12-05",
                    "2009-12-06",
                ),
...
class TestTar(BaseArchiveTestCase):
    ...
    @pytest.mark.skipif(
        on_windows, reason="Unicode and/or Long paths are not handled well yet on windows"
    )
...
class TestAr(BaseArchiveTestCase):
    ...
    def test_extract_ar_broken_libarchive(self):
        ...
        self.assertRaisesInstance(
            Exception("Incorrect file header signature"), libarchive2.extract, test_file, test_dir
        )
        expected = ["__.SYMDEF", "main.o"]
        check_files(test_dir, expected)
    ...
    def test_extract_ar_with_relative_path_sevenzip(self):
        ...
        test_dir = self.get_temp_dir()
        result = sevenzip.extract(test_file, test_dir)
        expected = ["1.txt", "2.txt", "release/init.obj"]
        check_files(test_dir, expected)
        assert [] == result
    ...
    def test_extract_ar_with_relative_path_libarch(self):
        ...
        test_dir = self.get_temp_dir()
        self.assertRaisesInstance(
            Exception("Invalid string table"), archive.libarchive2.extract, test_file, test_dir
        )
...
class TestCpio(BaseArchiveTestCase):
    ...
    def test_extract_cpio_broken_7z(self):
        ...
        test_dir = self.get_temp_dir()
        self.assertRaisesInstance(
            Exception("CRC Failed : elfinfo-1.0.tar"), sevenzip.extract, test_file, test_dir
        )
...
class TestRpm(BaseArchiveTestCase):
    ...
    def test_extract_rpm_broken(self):
        ...
        self.assertRaisesInstance(expected, archive.extract_rpm, test_file, test_dir)
...
class TestIso(BaseArchiveTestCase):
    def test_extract_iso_basic(self):
        ...
        extracted = self.collect_extracted_path(test_dir)
        expected = ["/ChangeLog", "/ChangeLog (copy)", "/freebase.ABOUT", "/this/", "/this/that"]
        assert sorted(expected) == sorted(extracted)
...
class TestXzLzma(BaseArchiveTestCase):
    def check_lzma_extract(self, extract_fun, test_file, expected):
        ...
        assert os.path.exists(expected_file), (
            ...
            "correctly from archive %(test_file)s" % locals()
        )

    def test_extract_archive_tar_xz_1(self):
        test_file = "archive/lzma_xz/texlive-core-patches-20.tar.xz"
        self.check_lzma_extract(
            extract_fun=archive.extract_lzma,
            test_file=test_file,
    ...
    def test_extract_archive_tar_lzma_2(self):
        ...
        self.check_lzma_extract(
            extract_fun=archive.extract_lzma, test_file=test_file, expected="orionsocket-1.0.9.tar"
        )
    ...
    def test_extract_archive_tar_lzma_3(self):
        ...
        self.check_lzma_extract(
            extract_fun=archive.extract_lzma, test_file=test_file, expected=expected
        )
...
class TestDia(BaseArchiveTestCase):
    ...
    def test_extract_dia_broken_1(self):
        ...
        self.assertExceptionContains(
            "CRC check failed", archive.uncompress_gzip, test_file, test_dir
        )
    ...
    def test_extract_dia_broken_2(self):
        ...
        test_dir = self.get_temp_dir()
        self.assertExceptionContains(
            "invalid distance too far back", archive.uncompress_gzip, test_file, test_dir
        )
...
class ExtractArchiveWithIllegalFilenamesTestCase(BaseArchiveTestCase):
    def check_extract_weird_names(
        ...
        extracted = sorted(
            path[len_test_dir:] for path in fileutils.resource_iter(test_dir, with_dirs=False)
        )
...
class TestExtractArchiveWithIllegalFilenamesWithLibarchiveOnPosix(
...
@pytest.mark.skipif(
    not on_windows, reason="Run only on Windows because of specific test expectations."
)
class TestExtractArchiveWithIllegalFilenamesWithLibarchiveOnWindows(
    ExtractArchiveWithIllegalFilenamesTestCase
):
...
@pytest.mark.skipif(
    on_windows or on_mac, reason="Do not run windows and mac because of specific test expectations."
)
class TestExtractArchiveWithIllegalFilenamesWithSevenzipOnPosix(
    ExtractArchiveWithIllegalFilenamesTestCase
):
...
class TestExtractArchiveWithIllegalFilenamesWithSevenzipOnMac(
    ExtractArchiveWithIllegalFilenamesTestCase
):
    ...
    def test_extract_rar_with_weird_filenames_with_sevenzip_macos(self):
        test_file = self.get_test_loc("archive/weird_names/weird_names.rar")
        self.check_extract_weird_names(
            sevenzip.extract, test_file, expected_warnings=[], expected_suffix="7zip", regen=False
        )
...
class TestExtractArchiveWithIllegalFilenamesWithSevenzipOnWin(
    ExtractArchiveWithIllegalFilenamesTestCase
):
...
class TestExtractArchiveWithIllegalFilenamesWithSevenzipOnWinWarning(
    ExtractArchiveWithIllegalFilenamesTestCase
):
    def test_extract_7zip_with_weird_filenames_with_sevenzip_win(self):
        test_file = self.get_test_loc("archive/weird_names/weird_names.7z")
...
class TestZipSlip(BaseArchiveTestCase):
    def test_extract_zipslip_zip_posix(self):
        ...
        if on_windows:
            try:
                ...
                expected = ["good.txt", "tmp/evil.txt"]
                check_files(test_dir, expected)
        else:
            check_files(test_dir, expected)
    ...
    def test_extract_zipslip_zip_win(self):
        ...
        if on_windows:
            try:
                ...
                expected = ["Temp/evil.txt", "good.txt"]
                check_files(test_dir, expected)
        else:
            check_files(test_dir, expected)
    ...
    @pytest.mark.skipif(
        on_windows,
        reason="Fails with WindowsError: [Error 206] The filename or extension is too long:",
    )
